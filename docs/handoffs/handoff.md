...
使用 `/grill-me`，一次只問一個上游問題，不要一次丟出全部未決項。第一題直接對齊核心檢定：

> 是否採用「基本擲 1d6；屬性符合 `＋1`；特殊能力實際用到且對應該屬性再 `＋1`；後援／寶物才提供多骰取最高；總分達到 DC 即成功」？

推薦答案是採用，理由是它最接近 D&D 的 modifier＋Advantage，同時只要求 5 歲孩子做 `骰面＋0～2`。若不採用，再比較「屬性／技能增加骰子取最高」與現行「每顆 4–6 計成功數」，不要直接發明第四套公式。

核心公式確認後，依序只處理：

1. 一般關只用 DC 4，或允許 DM 對高風險方法使用 DC 5。
2. 第二關雙位置如何互助。
3. Boss 用兩次檢定、累積成功挑戰，或保留部分成功數。
4. 萬用背包、探險手電筒、變化魔鏡與兩次後援如何映射。
5. 做概率試算與 5 歲／9 歲／兄弟合作文字模擬。

每確認一項就立即寫回方向稿的「已決定／尚待決定」，但在 Boss 與寶物一起定案前不要改 code、PDF 或揭露卡。
