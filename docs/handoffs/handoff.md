...
> 狀態：v0.8-M 四情境成人測試完成，自由方法與裁定初步通過；使用者不做刻意提示短測，提示維持未驗證；下一步可跑完整成人紙筆局

## 下個 session 從這裡開始

先讀 [`../plans/2026-08-04-team-mission-v08m-hidden-prompt-adult-test.md`](../plans/2026-08-04-team-mission-v08m-hidden-prompt-adult-test.md) 的本輪結論。自由方法、裁定、技能與簡單回答都已走完並初步通過；「同時揭露兩個備選提示」未觸發，使用者選擇不做刻意短測。下一步可把此風險帶入完整成人紙筆局，只有自然卡住時才觀察提示。前一輪成人代演、3 Agent 審查及停止的 H／M 測試見 [`../plans/2026-07-31-team-mission-v08-adult-dry-run.md`](../plans/2026-07-31-team-mission-v08-adult-dry-run.md)、[`../plans/2026-07-31-team-mission-v08-child-playability-review.md`](../plans/2026-07-31-team-mission-v08-child-playability-review.md)、[`../plans/2026-08-04-team-mission-v08m-adult-ab-test.md`](../plans/2026-08-04-team-mission-v08m-adult-ab-test.md)。

1. 固定使用非洲獅、紅袋鼠、海獺、蜂鳥；每題重新放回，不使用 DM 網頁、Boss、寶物、後援或卡片移區。
2. 四個情境都由 DM 只念危機與目標；玩家從手牌提出暫定動物與自由方法。每題使用預放骰面 `3`。
3. DM 先對照兩條隱藏預寫基準；真正不同的方法才用固定屬性／需求規則裁定。屬性符合便直接沿用；不符合時才提醒可繼續嘗試或免費換卡。
4. DM 自行判斷卡住時機；需要提示便同時揭露兩條預寫方法，玩家仍可提出第三種方法。
5. 記錄玩家原話、追問、提示、裁定來源、正式選卡、技能必要步驟、DM 負擔及是否存在至少兩個實質選擇。
6. 使用者已確認不讓孩子反覆學習未定案規格；成人測試只能排除明顯問題，不能當作兒童理解驗證。

## 接手後先做什麼

不要再重開已確認的核心公式、DC、Boss 或寶物討論；下一個有效證據應來自完整成人紙筆局：

1. 不再追加 Agent 模擬，也不讓孩子參與候選版反覆測試。
2. 不再執行已停止的 H／M 其餘七題，也不追加刻意提示短測。
3. 依本輪結果改寫六個正式位置的十二條預寫基準，再跑一局完整成人紙筆《暴雨森林救援隊》；若自然卡住才使用兩個提示。
4. 完整局仍不能驗證實際兒童理解；紙筆規格穩定前不做 Web prototype。
...
- 7/30 曾確認純 v0.8 類 D&D 流程：孩子先說目標與行動，DM 再判斷屬性與工作尺度；7/31 成人 dry run 後，這個隱藏動態裁定已暫停，下一候選改為先公開固定區間與兩個常見屬性的 v0.8-H。
- 核心公式定案為 `1d6＋屬性符合 1 分＋特殊能力直接處理工作且對應所選屬性 1 分 ≥ DC 4`；一般關與 Boss 均固定 DC 4。
- 5 歲與 9 歲採三句故事接力：卡片主人說行動、另一位孩子補一個細節、DM 說明屬性與 DC；哥哥不能替弟弟改動物或決定出場。
- 第二關與 Boss 均為 A、B 各一次檢定且兩邊都成功；只有一邊失敗時才可用全隊後援。兩枚後援由前三關與 Boss 共用。
- 三寶物改為直接作用核心檢定：背包指定一邊 Advantage、手電筒把一個總分 3 改成 4、魔鏡重試任一失敗位置且該位置不能再接受後援。
- `+1／+1` 且後援也是 `+1` 時，Boss 隨機取得 0／1／2／3 件寶物的通關率為 74.1%／83.3%／88.9%／93.1%；無後援時為 44.4%／66.7%／83.5%／92.9%。
- 完整規則、概率、現行劇情映射、真人記錄欄位與實作骨架已整理於 [`../plans/2026-07-22-team-mission-dnd-check-design.md`](../plans/2026-07-22-team-mission-dnd-check-design.md)。
- 最終 5 歲／9 歲／兄弟合作 3 Agent 模擬均無規則死結；但成人 dry run 顯示純 v0.8 的數字可見性與 DM 裁定負擔有問題，舊完整局試玩單已暫停。
- v0.8-H 第一題再次出現「為什麼聰明只能 4、5、6」後停止；不同屬性不能為了版面簡化而共用同一需求。
- v0.8-M 的兩條預寫方法改為 DM 隱藏裁定基準；玩家先自由想，DM 公布裁定後可免費換卡。DM 判斷卡住時，兩條基準才同時顯示為提示。
- 免費換卡是屬性不符合時的保護權利，不是每回合固定確認；屬性符合便直接結算，技能沒有 `＋1` 不單獨觸發換卡詢問。
- 提示只顯示具體動作、屬性與數字，不顯示精細／適中／標準／強力等模板名。設計端只採 `1–3`、`4–6`、`4–9`、`7–9` 四種需求；`4–6` 只在過低與過高都確實會出錯時使用。
- 使用者決定由自己進行成人代理測試，不讓孩子反覆學習變動中的規格；成人代理只能排除明顯問題。
- 現行 v0.7 保留為比較基準；本輪仍沒有修改 `game/dm/`、揭露卡、寶物或規則程式。
...
1. 四情境成人測試已完成；自由方法與裁定初步通過，提示未驗證。
2. 下一步跑完整成人《暴雨森林救援隊》，特別記錄十二條預寫基準、技能必要步驟、魔鏡／後援、Boss 感受，以及提示是否自然觸發。
3. 完整紙筆規則穩定後，才讓孩子學接近定案的版本，並決定何時修改 DM 網頁與實體揭露卡。
...
- 新 branch 從已完成的 `feat/animal-ability-audit` 切出；7/30 完成純 v0.8 規則與概率，7/31 完成成人 dry run 與分齡審查，8/4 停止 H／M 比較並確認 v0.8-M 隱藏基準／提示制成人測試。
- 接手時先執行 `git status -sb`；若本輪文件尚未提交，預期會看到 `.gitignore`、方向稿與本接手文件的修改。
