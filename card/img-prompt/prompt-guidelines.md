...
撰寫 prompt 前，必須先列出每隻動物 **2-3 個最高辨識度的外觀特徵**，確保全部寫進 prompt。

### 研究重點

| 項目 | 說明 | 範例 |
|------|------|------|
| **區別近親的特徵** | 該動物與同科近親最大的外觀差異 | 黑豹 vs 花豹：幽靈玫瑰花紋 + 全身粗輪廓 |
| **招牌體態** | 該動物最被人記住的身體部位 | 鼷鹿：極小體型 + 上犬齒外露 |
| **毛色/紋理在線稿中的表現** | 若毛色是辨識關鍵，需轉換為線稿可呈現的描述 | 黑豹：`solid coat in bold thick outline` |
...
### 0.5 先固定 `artAction`，不要直接把 `skillDesc` 丟給模型

`skillDesc` 是給孩子閱讀與說故事的卡面文案，可以包含用途、效果或情境；生圖前必須另外整理一條製作用的 `artAction`，只描述畫面真正要出現的單一瞬間。

```text
skillName：巧手造物
skillDesc：會挑選細樹枝當工具，伸進小洞找出藏在裡面的昆蟲。
artAction：紅毛猩猩蹲坐，用單根細樹枝探入地面小洞。
```

`artAction` 必須符合：

- 只有一個主要動詞與一個動作方向。
- 預設只有一隻主角動物；群體本身是能力核心時才例外。
- 最多一個簡單互動物件；經審核的必要因果組合才可使用兩個。
- 不寫敵人、獵物、受害者、傷口、死亡或看不見的結果。
- 不寫「很厲害、完全隱形、所向無敵」等抽象效果，改寫成身體姿態或可見結果。
- 修改 `skillDesc` 時，只要核心能力與 `artAction` 沒變，就不必重畫；若主要動詞、身體部位或互動物件改變，必須重新審圖。
...
- [ ] **正面可認**：動物的臉是否面向或側向觀眾？若技能動作導致背對（如堵洞、鑽地），須改用側面視角或加 `head turned toward the viewer` 確保可辨識。
- [ ] **風格一致**：是否包含完整的風格後綴？
...
黑白線稿中，毛色無法直接呈現。若動物的毛色是辨識關鍵，必須用「結構性花紋」替代：

| 動物類型 | 問題 | 處理方式 |
|----------|------|---------|
| 黑化動物（黑豹、黑美洲豹） | 全身黑色在線稿中無區分度 | 加 `faint ghost [原始花紋] pattern visible within the outline` |
| 斑紋動物（斑馬、虎） | 線稿中斑紋可能不明顯 | 加 `bold [stripe/spot] pattern` 強調紋理 |
| 純白動物（北極熊、雪鴞） | 白色在白底上消失 | 用 `bold outline` + 毛髮質感線條區分 |
