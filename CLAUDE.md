...
- 動物大對決（battle-rules.md）：v2.3 數值系統（天賦為 +4 分配，總和 11-38）
- 兩模式獨立運作，待實際遊玩測試後再決定是否統一

### 數值設計準則（v2.3）
- **寫實優先**：依動物真實能力給分，允許極端差距（虎鯨 38 vs 蘭花螳螂 11）
- 5 屬性總和：11-38（依動物強度分級）
- 弱項：至少 1 個 ≤ 3
- 亮點：至少 1 個 ≥ 7
...
100,000 次模擬，原規則（各自擲骰）

| 排名 | 動物 | 總和 | 1v1 勝率 | 3 強機率 | 冠軍率 |
|:----:|------|:----:|--------:|--------:|------:|
| 1 | 大白鯊 | 32 | 72.19% | 20.34% | 9.71% |
| 2 | 北極熊 | 31 | 70.39% | 17.82% | 7.93% |
| 3 | 河馬 | 31 | 68.25% | 15.86% | 6.66% |
| 4 | 犀牛 | 31 | 67.41% | 15.35% | 6.47% |
| 5 | 藍鯨 | 31 | 67.16% | 15.33% | 6.33% |
| 6 | 非洲獅 | 30 | 68.34% | 15.11% | 5.98% |

> 詳細數據見 `sim/results/v4_*.csv`
> 舊版 32 隻淘汰賽見 `sim/results/original_rules_*.csv`
